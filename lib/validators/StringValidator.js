...
class StringValidator extends Validator {
  ...
  */
  validate(str) {
    ...
    let detail = `entre ${this.min} y ${this.max} caracteres`
    if (this.min == this.max) {
      detail = (this.min == 1) ? `de 1 caracter` : `de ${this.min} caracteres`
    }
    return { isValid:false, value:str, message:`Debe ser una cadena de texto ${detail}` }
  }
  ...
  */
  apidocType() {
    if (this.min == this.max) {
      return (this.min == 1) ? `String{1 caracter}` : `String{${this.min} caracteres}`
    }
    return `String{de ${this.min} a ${this.max} caracteres}`
  }
