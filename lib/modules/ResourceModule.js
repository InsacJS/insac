...
class ResourceModule extends Module {
  constructor (moduleName, modulePath, config = {}) {
    ...
    this.config = {
      schema      : config.schema      || moduleName.toLowerCase(),
      seedersPath : config.seedersPath || path.resolve(modulePath, 'seeders'),
      modelsPath  : config.modelsPath  || path.resolve(modulePath, 'models'),
      routesPath  : config.routesPath  || path.resolve(modulePath, 'routes'),
      daoPath     : config.daoPath     || path.resolve(modulePath, 'dao')
    }
  }
  async onSetup (app) {
    await super.onSetup(app)
    await app.DB.import(this.moduleName, { modelsPath: this.config.modelsPath, daoPath: this.config.daoPath })
    await app.DB.setup(this.moduleName, { seedersPath: this.config.seedersPath })
  }
  async onStart (app) {
