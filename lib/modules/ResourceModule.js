...
class ResourceModule extends Module {
  ...
  */
  async onSetup (app) {
    ...
    if (this.config.modelsPath)   await app.DB.import(this.config.moduleName, { modelsPath: this.config.modelsPath, daoPath: this.config.daoPath })
    if (this.config.modelsPath)   await app.DB.setup(this.config.moduleName, { seedersPath: this.config.seedersPath })
  ...
  */
  _loadRoutes (app, MODULE) {
    ...
    util.find(MODULE.config.resourcesPath, '.route.js', ({ filePath, fileName, dirPath }) => {
      ...
      Object.keys(ROUTES).forEach(key => {
        ...
        let permissions = []
        MIDDLEWARE[key].forEach(mw => { if (mw.roles) { permissions = mw.roles } })
        ROUTE.key          = key
        ...
        app.log(`\x1b[2m${_.padEnd(` [${ROUTE.method.toUpperCase()}]`, 9, ' ')} ${_.padEnd(`${ROUTE.path} `, pathSize + 1, '.')}... ${ROUTE.key}\x1b[0m\n`)
      })
      app.log('\n')
  ...
  /**
  * Función encargada de cargar todos los servicios de un recurso.
  * @param {Function}       app    - Instancia del servidor express.
  * @param {ResourceModule} MODULE - Instancia del módulo.
  */
  _loadServices (app, MODULE) {
    ...
    util.find(MODULE.config.servicesPath, '.service.js', ({ filePath, fileName }) => {
      MODULE.services[fileName] = require(filePath)(app)
    })
  }
}
