    ...
    try {
      ...
      if (RESOURCE.middleware[key]) {
        if (Array.isArray(RESOURCE.middleware[key])) {
          RESOURCE.middleware[key].forEach(async middleware => {
            if (res.headersSent === false) { await middleware(req, res) }
          })
        } else { if (res.headersSent === false) { await RESOURCE.middleware[key](req, res) } }
      }
      if (res.headersSent === false) { await RESOURCE.controller[key](req, res) }
