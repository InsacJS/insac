...
function _createMiddlewares (RESOURCE, key) {
  const middlewares = []
  RESOURCE.defaultMiddleware[key] = RESOURCE.defaultMiddleware[key] || []
  RESOURCE.middleware[key] = RESOURCE.middleware[key] || []
  ...
  RESOURCE.defaultMiddleware[key].forEach(MW => { middlewares.push(MW) })
  RESOURCE.middleware[key].forEach(MW => { middlewares.push(MW) })
  return middlewares
}
