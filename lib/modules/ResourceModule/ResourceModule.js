...
function _updateFieldsWithGroup (app, properties) {
  ...
  if (app.FIELD.models[resourceName]) {
...
function _createController (app, properties, RESOURCE) {
  ...
  return async (req, res) => {
    try {
      ...
      if (key === 'listar') { _updateQueryForList(QUERY, MODEL, defaultOrder, defaultLimit, defaultPage) }
      req.options = Options.create({ query: QUERY, output: properties.output, model: MODEL, keys: true })
      if (RESOURCE.middleware[key]) {
        if (Array.isArray(RESOURCE.middleware[key])) {
          RESOURCE.middleware[key].forEach(async middleware => { await middleware(req) })
        } else { await RESOURCE.middleware[key](req) }
      }
      ...
      await RESOURCE.controller[key](req, success)
