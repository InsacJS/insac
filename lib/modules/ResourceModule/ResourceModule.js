...
function addRoute (app, method, path, key, properties = {}, options) {
  ...
  properties.key = key
  properties.modelName = fileName
  properties.plainOutput = (typeof properties.plainOutput !== 'undefined') ? properties.plainOutput : options.plainOutput
  properties.input = Object.assign(_.clone(options.DEFAULT.input[key] || {}), options.INPUT[key] || {})
  properties.output = Object.assign(_.clone({ body: options.DEFAULT.output[key] || {} }), { body: options.OUTPUT[key] || {} }).body
  _updateFieldsWithGroup(app, properties, fileName)
  ...
  properties.controller = _createController(app, properties, options)
  app.APIDOC[method](path, properties)
}

function _updateFieldsWithGroup (app, properties, modelName) {
  if (app.FIELD.models[modelName]) {
    if (properties.input.headers) { properties.input.headers = app.FIELD.group(modelName, properties.input.headers) }
    if (properties.input.params) { properties.input.params = app.FIELD.group(modelName, properties.input.params) }
    if (properties.input.query) { properties.input.query = app.FIELD.group(modelName, properties.input.query) }
    if (properties.input.body) { properties.input.body = app.FIELD.group(modelName, properties.input.body) }
    properties.output = app.FIELD.group(modelName, properties.output)
  }
}

function _createController (app, properties, options) {
  return async (req, res) => {
    try {
      const key = properties.key
      const QUERY = req.query || {}
      const MODEL = app.DB.models[properties.modelName]
      if (key === 'listar') { _updateQueryForList(app, QUERY, MODEL) }
      req.options = Options.create({ query: QUERY, output: properties.output, model: MODEL, keys: true })
      ...
      const success = (data, message, count) => {
        data = Options.filter(data, { query: QUERY, output: properties.output, plainOutput: properties.plainOutput })
