...
module.exports = (app) => {
  ...
  MIDDLEWARE.obtener = async (req) => {
    ...
    if (!await app.DAO.usuario_rol.buscar({id_usuario_rol: ID_USUARIO_ROL})) {
      throw new errors.PreconditionError([{
        path: 'params.id',
        value: ID_USUARIO_ROL,
        msg: `No se encuentra el registro usuario_rol solicitado.`
      }])
    }
  }
  ...
  MIDDLEWARE.crear = async (req) => {
    ...
    if (await app.DAO.usuario_rol.buscar({ fid_usuario: FID_USUARIO, fid_rol: FID_ROL })) {
      throw new errors.PreconditionError([{
        path: 'body.fid_usuario | body.fid_rol',
        value: `${FID_USUARIO} | ${FID_ROL}`,
        msg: `El usuario y el rol ya se encuentran registrados.`
      }])
    }
  }
  ...
  MIDDLEWARE.actualizar = async (req) => {
    ...
    if (!await app.DAO.usuario_rol.buscar({ id_usuario_rol: ID_USUARIO_ROL })) {
      throw new errors.PreconditionError([{
        path: 'params.id',
        value: ID_USUARIO_ROL,
        msg: `No se encuentra el registro usuario_rol que desea actualizar.`
      }])
    }
  ...
  MIDDLEWARE.eliminar = async (req) => {
    ...
    if (!await app.DAO.usuario_rol.buscar({ id_usuario_rol: ID_USUARIO_ROL })) {
      throw new errors.PreconditionError([{
        path: 'params.id',
        value: ID_USUARIO_ROL,
        msg: `No se encuentra el registro usuario_rol que desea eliminar.`
      }])
    }
  }
