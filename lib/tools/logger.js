...
const winstonLogger = createLogger({
  levels      : { error: 0, warn: 1, info: 2, verbose: 3, debug: 4, silly: 5, trace: 6, notice: 7 },
  transports  : TRANSPORTS,
  exitOnError : false
})

/**
* Objeto que contiene funciones que imprimen un mensaje por consola.
* @type {Object}
*/
const logger = {
...
logger.requestHeaders = (req) => {
  ...
  if (Object.keys(req.headers).length > 0) logger.info(MESSAGE)
}
...
logger.requestError = (req, err) => {
  ...
  if (err.code === 500) { logger.error(req, MESSAGE) } else { logger.warn(req, MESSAGE) }
}
...
logger.session = (req, session) => {
  let MESSAGE = `[SESION] `
  if (session.usuario && session.usuario.id && session.usuario.nombre && session.usuario.roles) {
    MESSAGE += `ID USUARIO: ${session.usuario.id}  ROLES: ${session.usuario.roles}  NOMBRE: ${session.usuario.nombre}`
  } else {
...
*/
logger.responseData = (req, data = {}) => {
  process.logger.write(LIGHT_GREEN_COLOR)
  const MESSAGE = `[DATA] = ${JSON.stringify(data, null, 2)}`
  if (Object.keys(data).length > 0) logger.notice(req, MESSAGE)
}
