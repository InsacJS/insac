...
class SeedCreator {
  ...
  */
  static async create (model, data, options = {}) {
    ...
    options.logger    = options.logger  || { appVerbose: (title = '', message = '') => console.log(title, message), OK: '', FAIL: 'x' }
...
async function _bulkInsert (model, data, options) {
  ...
  options.logger.appVerbose('BULK INSERT', `${schema}${model.name} [${data.length} registros] ...\n`)
...
async function _insert (model, data, options) {
  ...
  try {
    ...
    const ID     = RESULT[PK]
    await _updateAutoIncrement(model, PK, options)
    options.logger.appVerbose('INSERT', `[${++SeedCreator.cnt}/${SeedCreator.total}] ${schema}${model.name} .... [${PK}: ${ID}] ${options.logger.OK}`)
    return ID
  } catch (e) {
