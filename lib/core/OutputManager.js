...
class OutputManager {
  ...
  createMetadata(req, route, db) {
    return new Promise((resolve, reject) => {
      async function task() {
        try {
          let options = { }
          // TODO incluir todos los Where, no solamente el primero
          if (req.options.where) options.where = req.options.where
          let total = await db[route.model.name].count(options)
          let metadata = {
  ...
  * @return {Object}
  */
  _createOutputTemplate(queryFields = "all", route, models) {
    if (queryFields == 'ALL') {
      queryFields = this._createQueryFieldsALL(route.output)
    }
    let queryOutput = this._queryTemplate(queryFields, route, models)     // queryOutput
  ...
  * @return {Object}
  */
  _queryTemplate(queryFields = "all", route, models) {
    let queryTemplate = { }
      ...
      if (outputTemplate[prop] instanceof Field) {
        ...
        if (query[finalTrace]) {
          if (prop == 'id') {
            ...
            for (let i in ids) {
              let { isValid, value, message } = outputTemplate[prop].validate(ids[i])
              if (!isValid) {
