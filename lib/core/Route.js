  ...
  */
  init(app) {
    ...
    for (let i in this.middlewares) {
      ...
      if (typeof middlewareRoute == 'function') {
        this.middlewares[i] = { callback:middlewareRoute }
        continue
      }
  ...
  */
  _copyMiddlewarePropertiesOnRoute(origin, destiny, trace, routePath, middlewareName) {
    for (let prop in origin) {
      if (destiny[prop]) {
        let msg = `(insac) Advertencia.- Se sobreescribió el campo '${trace}${prop}' de la ruta '${routePath}'
               con el valor que se encuentra en el middleware '${middlewareName}',
               se recomienda declarar este campo solamente en el middleware.`
