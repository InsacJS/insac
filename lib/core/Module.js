...
class Module {
  ...
  */
  async onInit (app) {
    ...
    if (Object.keys(MODULE.models).length > 0) app.logger.appVerbose()
  ...
  */
  async onStart (app) {
    ...
    await MODULE.loadComponent(app, 'mails', '.mail.js')
    await MODULE.loadComponent(app, 'reports', '.report.js')
    await MODULE.loadComponent(app, 'storage', '.storage.js')
    await MODULE.loadComponent(app, 'services', '.service.js')
  ...
  */
  async loadComponent (app, dirName, fileExt, onFind) {
    ...
    if (util.isDir(dirPath)) {
      ...
      if (fileLoaded) app.logger.appVerbose()
    }
...
async function loadResource (app, MODULE, { filePath, fileName, dirPath }) {
  ...
  if (HAS_RESOURCES) app.logger.appVerbose()
  Object.keys(RESOURCE).forEach(key => {
    ...
    app.logger.appVerbose('[ruta]', `${routeMethod} ${routePath}... ${routeKey} ${app.logger.OK}`)
  })
...
* @param {!Object}   data - Datos del objeto input u output.
