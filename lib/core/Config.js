...
class Config {
  ...
  * @param {Object} [properties] Objecto que contiene todos los parámetros de configuración.
  */
  constructor(properties = {}) {
    ...
    */
    this.env = process.env.NODE_ENV = properties.env || 'development'

    /**
    * Ruta del directorio raiz del proyecto.
    * @type {String}
    */
    this.projectPath = properties.projectPath || Config.DEFAULT_PROJECT_PATH
  ...
  update(config = {}) {
    ...
    if (config.path) {
      for (let prop in config.path) {
        if (typeof config.path[prop] != 'undefined') this.path[prop] = path.resolve(this.projectPath, config.path[prop])
      }
    }
    if (config.server) {
      for (let prop in config.server) {
        if(typeof config.server[prop] != 'undefined') this.server[prop] = config.server[prop]
      }
      if (config.server[this.env]) {
        for (let prop in config.server[this.env]) {
          if(typeof config.server[prop] != 'undefined') this.server[prop] = config.server[this.env][prop]
        }
      }
    }
    if (config.database) {
      ...
      if (config.database[this.env]) {
        for (let prop in config.database[this.env]) {
          if(typeof config.database[this.env][prop] != 'undefined') this.database[prop] = config.database[this.env][prop]
        }
      }
    }
    if (config.auth) {
    ...
    if (config.apidoc) {
      ...
      if (config.apidoc[this.env]) {
        for (let prop in config.apidoc[this.env]) {
          if(typeof config.apidoc[prop] != 'undefined') this.apidoc[prop] = config.apidoc[this.env][prop]
        }
      }
    }
  }
