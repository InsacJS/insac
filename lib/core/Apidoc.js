...
class Apidoc {
  ...
  */
  _successMiddleware (route) {
    const responseData = (process.env.RESPONSE_LOG && (process.env.RESPONSE_LOG === 'true')) ? (...args) => logger.responseData(...args) : () => {}
    const responseCode = (!process.env.REQUEST_LOG || (process.env.REQUEST_LOG === 'true'))  ? (...args) => logger.responseCode(...args) : () => {}
  ...
  */
  async build () {
    ...
    try {
      await util.cmd(`"../../apidoc/bin/apidoc" -i "doc" -o "${output}"`, tmpPath)
    } catch (e) {
      try {
        await util.cmd(`"../node_modules/apidoc/bin/apidoc" -i "doc" -o "${output}"`, tmpPath)
      } catch (e) {
        throw new Error(`ApidocJS no se encuentra instalado: Ejecute 'npm install' para solucionarlo.`)
      }
    }
  ...
  _updateIndex (output) {
    ...
    if (!util.isFile(INDEX_PATH)) {
      if (!process.env.LOG || process.env.LOG === 'true') {
        console.log(` No se encontraron rutas para generar el APIDOC.\n`)
      }
      return
    }
