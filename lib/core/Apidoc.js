...
class Apidoc {
  ...
  */
  _successMiddleware (route) {
    function onSuccess (result, req) {
      result.data = Options.filter(result.data, {
        query  : req.query,
        output : route.output,
        plain  : route.plain
      })
  ...
  */
  async build () {
    ...
    const outPath = insacConfig.PATH.public
    util.mkdir(tmpPath)
    util.mkdir(outPath)
    if (insacConfig.APIDOC.header) {
      let HEADER_PATH     = path.resolve(insacConfig.PATH.project, `./${insacConfig.APIDOC.header.filename}`)
    ...
    if (insacConfig.APIDOC.footer) {
      let FOOTER_PATH     = path.resolve(insacConfig.PATH.project, `./${insacConfig.APIDOC.footer.filename}`)
    ...
    util.writeFile(path.resolve(tmpPath, 'doc/apidoc.js'), this.apidocContent)
    const output = path.resolve(outPath, 'apidoc')
    try { util.rmdir(output) } catch (e) { }
    await this._createApidoc('doc', output, tmpPath)
    await this._updateIndex(output)
  ...
  * @return {Promise}
  */
  async _createApidoc (input, output, execPath) {
    try {
      await util.cmd(`node "../../apidoc/bin/apidoc" -i "${input}" -o "${output}"`, execPath)
    } catch (e) {
      try {
        await util.cmd(`node "../node_modules/apidoc/bin/apidoc" -i "${input}" -o "${output}"`, execPath)
      } catch (e) {
        try {
          await util.cmd(`apidoc -i "${input}" -o "${output}"`, execPath)
        } catch (e) {
  ...
  */
  remove () {
    try { util.rmdir(path.resolve(insacConfig.PATH.public, './apidoc')) } catch (e) { }
  }
  ...
  */
  _updateIndex (output) {
    ...
    const EMPTY_APIDOC_PATH = path.resolve(__dirname, '../apidoc/empty')
    if (!util.isFile(INDEX_PATH)) {
      util.copyDir(EMPTY_APIDOC_PATH, output)
      return logger.app(` No se encontraron rutas para generar el APIDOC.\n`)
    }
