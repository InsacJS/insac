...
class Apidoc {
  ...
  */
  async build () {
    ...
    this.app.MODULES.forEach(mod => {
      if (this.app[mod].config.moduleType === 'RESOURCE') {
        util.writeFile(path.resolve(tmpPath, `doc/${mod}/apidoc.js`), this.apidocContent[mod] || '')
      }
    })
    const output = insacConfig.PATH.apidoc
    try { util.rmdir(output) } catch (e) { }
    for (let i in this.app.MODULES) {
      const mod = this.app.MODULES[i]
      if (this.app[mod].config.moduleType === 'RESOURCE') {
        logger.app(`\x1b[2m [apidoc] módulo ${mod} \x1b[0m\u2713`)
        await this._createApidoc(`doc/${mod}`, `${output}/${mod}`, templatePath, tmpPath)
        await this._updateIndex(output, mod)
      }
    }
    logger.app('')
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (typeof e === 'string' && e.includes('apidoc: not found')) {
  ...
  */
  remove () {
    try { util.rmdir(insacConfig.PATH.apidoc) } catch (e) { }
  }
  ...
  */
  _updateIndex (output, mod) {
    ...
    if (!util.isFile(INDEX_PATH)) {
      ...
      emptyContent = emptyContent.replace('{{INDEX_PAGE_URL}}', `${insacConfig.APIDOC.url}`)
