  ...
  */
  ...
  app.APIDOC = Apidoc.router((route, apidoc) => {
    function onSuccess (data, req) {
      data.data = Options.filter(data.data, { query: req.query, output: route.output, plainOutput: route.plainOutput }) || null
      return data
    }
    app[route.method](
      route.path,
      Response.success({ successFormat: app.successFormat, onSuccess }),
