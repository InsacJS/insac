...
class Model {
  ...
  * en la propiedad output de una ruta.
  ...
  */
  _attributesOptions(output) {
    let attributes = []
    for (let fieldName in this.fields) {
      if ((typeof output[fieldName] != 'undefined') || (fieldName == 'id')) {
  ...
  */
  ...
  /**
  * Devuelve el nombre del modelo con la primera letra en mayúsculas.
  * @return {String}
  */
  getName() {
    let name = this.name
    let firstLetter = name.substr(0, 1)
    return firstLetter.toUpperCase() + name.substr(1)
  }
