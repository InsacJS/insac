...
class Model {
  ...
  */
  constructor(name, properties = {}) {
    ...
    */
    ...
    for (let prop in properties.fields) {
      this.fields[prop] = properties.fields[prop]
    }
  ...
  /**
  * Devuelve el nombre de la clave primaria.
  * @return {String}
  */
  getPK() {
  ...
  /**
  * Inicializa el modelo. Actualiza las propiedades de los campos con referencias
  * y la propiedad associations de los otros modelos si es necesario.
  * @param {Model[]} models Colección de modelos.
  */
  init(models) {
    let associationModel = this
    for (let prop in this.fields) {
      let field = this.fields[prop]
      if (field instanceof Reference) {
        let referenceModel = models[field.reference.model]
        if (typeof referenceModel == 'undefined') {
          throw new Error(`MODEL_NOT_FOUND ${field.reference.model}`)
        }
        let model = models[field.reference.model]
        let key = field.reference.key || model.getPK()
        field.type = model.fields[key].type
        field.example = model.fields[key].example
        field.validator = model.fields[key].validator
        field.description = field.description || `${model.fields[key].description} del registro '${model.name}'`
        this.fields[prop].update(referenceModel, associationModel, prop)
      }
    }
  }
  ...
  _includeOptions(output, models, db) {
    ...
    for (let fieldName in this.fields) {
      ...
      if (field instanceof Reference) {
        ...
        let referenceAs = field.reference.as
        if (typeof output[referenceAs] != 'undefined') {
          let referenceOutput = output[referenceAs]
          let includeElement = {
            model: db[referenceModelName],
            as: field.reference.as,
  ...
  */
  getModelOfProperty(prop, models) {
    ...
    for (let fieldName in this.fields) {
      let field = this.fields[fieldName]
      if (field instanceof Reference) {
        let model = (field.reference.as == prop) ? models[field.reference.model] : undefined
  ...
  */
  ...
  /**
  * Devuelve información del modelo en formato de texto plano (markdown) para la documentación.
  * @return {String}
  */
  apidoc() {
    let content = `\n### ${this.name}\n${this.description}\n\n`
    content += `| Atributo | Tipo de dato | Descripción |\n|--|--|--|\n`
    for (let prop in this.fields) {
      let field = this.fields[prop]
      let name = prop
      let pk = (field.primaryKey) ? '[ PK ]' : ''
      let fk = (field instanceof Reference) ? '[ FK ]' : ''
      let type = field.type.name
      let description = field.description
      content += `| \`${name}\` ${pk}${fk} | \`${type}\` | ${description} |\n`
    }
    return content
  }
