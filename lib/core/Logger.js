...
/** @ignore */ const colors = require('../utils/theme').colors
...
class Logger {
  ...
  */
  appLogo (app) {
    ...
    msg += ` ${colors.NOTICE} Sistema :${colors.RESET}${colors.TEXT} ${app.config.PROJECT.appName}\n`
    msg += ` ${colors.NOTICE} Versión :${colors.RESET}${colors.TEXT} ${app.config.PROJECT.appVersion}\n`
    msg += ` ${colors.NOTICE} Entorno :${colors.RESET}${colors.TEXT} ${app.config.SERVER.env}\n\n\n`
...
function consoleFormat (app) {
  return (info) => {
    ...
    return `${app.logger.levelColors[RESULT.level]}${RESULT.timestamp}${RESULT.reqId}${LEVEL_STR} ${RESULT.message}${RESULT.data}${colors.RESET}`
  }
}
