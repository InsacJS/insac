...
class Database {
  ...
  */
  ...
  /**
  * Actualiza la actual instancia de Sequelize con una nueva.
  * Esto es muy útil cuando se requiere cargar nuevamente todos los modelos,
  * evitando de esta forma problemas con las asociaciones.
  */
  createSequelizeInstance () {
    this._verifyDialects()
    try {
  ...
  */
  _verifyDialects () {
    let packageName
    switch (config.DATABASE.params.dialect) {
      case 'postgres':
        try { require('pg') } catch (e) { if (e.message === 'Cannot find module') { packageName = 'pg' } }
        break
      case 'mysql':
        try { require('mysql2') } catch (e) { if (e.message === 'Cannot find module') { packageName = 'mysql2' } }
        break
      case 'mssql':
        try { require('tedious') } catch (e) { if (e.message === 'Cannot find module') { packageName = 'tedious' } }
        break
      case 'sqlite':
        try { require('sqlite3') } catch (e) { if (e.message === 'Cannot find module') { packageName = 'sqlite3' } }
        break
      default:
        throw new Error(`El dialecto ${config.DATABASE.params.dialect} no existe. Dialectos soportados: postgres, mysql, mssql y sqlite.`)
    }
    if (packageName) {
      throw new Error(`Se requiere el paquete ${packageName}. Ejecute "npm install --save ${packageName}" para solucionarlo.`)
    }
  }
  ...
  _updateForeignKeys (model) {
    ...
    Object.keys(model.attributes).forEach(fieldName => {
      ...
      if (FIELD.references) {
        ...
        FIELD.validate = F_MODEL.attributes[F_TARGET].validate
        FIELD.comment  = FIELD.comment || F_MODEL.attributes[F_TARGET].comment
        FIELD.example  = FIELD.example || F_MODEL.attributes[F_TARGET].example
      }
    })
  }
  ...
  */
  async setupModule (moduleName, options = {}) {
    ...
    APP.MODULES.forEach(modName => { if (APP[modName].config.setup && APP[modName].config.schema) { ALLOWED_SCHEMAS.push(APP[modName].config.schema) } })
    if (!ALLOWED_SCHEMAS.includes(SCHEMA)) {
      return logger.app(`\x1b[2m [Instalación Deshabilitada]\x1b[0m\n`)
    }
    await this._createTables(MODULE)
    if (!options.seedersPath) { return }
    if (config.SERVER.env === 'production') {
      options.seedersPath = path.resolve(options.seedersPath, 'production')
  ...
  async _createDatabase () {
    try {
      ...
      if (e.name === 'SequelizeConnectionError' && DATABASE_DOES_NOT_EXIST) {
        ...
        const PARAMS   = config.DATABASE.params
        ...
        const task1 = new Loader(` - \x1b[32mCREATE\x1b[0m\x1b[2m DATABASE ${DATABASE}`)
        task1.start()
  ...
  async _createSchemas (ALLOWED_SCHEMAS) {
    ...
    for (let i in ALLOWED_SCHEMAS) {
      if (!ALL_SCHEMAS.includes(ALLOWED_SCHEMAS[i])) {
        const task2  = new Loader(` - \x1b[32mCREATE\x1b[0m\x1b[2m SCHEMA ${ALLOWED_SCHEMAS[i]}`)
        task2.start()
  ...
  */
  async _createTables (MODULE) {
    for (let key in MODULE.models) {
      const task1 = new Loader(` - \x1b[0m\x1b[32mDROP\x1b[0m\x1b[2m TABLE ${MODULE.models[key].name}`)
      task1.start()
    ...
    async function _sync (model) {
      ...
      const task2 = new Loader(` - \x1b[0m\x1b[32mCREATE\x1b[0m\x1b[2m TABLE ${model.name}`)
      task2.start()
  ...
  */
  async _createEntries (MODULE, ALLOWED_SCHEMAS, { seedersPath, seedExt }) {
    ...
    const _seed = async (modelName, filePath) => {
          ...
          if (model.associations[i].associationType === 'BelongsTo') {
            if (!loaded.includes(model.associations[i].target.name)) {
              await util.findAsync(seedersPath, SEED_EXT, async ({ fileName, filePath, dirName }) => {
                if ((dirName !== 'prod' && dirName !== 'production') || config.SERVER.env === 'production') {
      ...
      logger.app(`\x1b[2m Insertando registros desde el fichero \x1b[0m${modelName}${SEED_EXT}\x1b[2m ...\x1b[0m`)
      const DATA = await require(filePath)(APP)
      try {
