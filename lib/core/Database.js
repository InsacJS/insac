  ...
  */
  updateModels(model, models) {
    ...
    for (let prop in model.fields) {
      ...
      let associationAs = prop.substr(3)
      if (field instanceof Reference) {
        let nameB = field.reference.model, referenceAs
        switch (field.reference.type) {
          case '1:1':
            referenceAs = model.options.singular
            associationAs = field.reference.as
            if (!temp.includes(nameB)) {
              temp.push(nameB)
              //console.log(`${nameB}.hasOne(${nameA}, {as:${referenceAs}, foreignKey:${prop}})`)
              this.sequelizeModels[nameB].hasOne(this.sequelizeModels[nameA], {as:referenceAs, foreignKey: prop})
            }
            break
          case '1:N':
            referenceAs = model.options.plural
            associationAs = field.reference.as
            if (!temp.includes(nameB)) {
              temp.push(nameB)
              //console.log(`${nameB}.hasMany(${nameA}, {as:${referenceAs}, foreignKey:${prop}})`)
              this.sequelizeModels[nameB].hasMany(this.sequelizeModels[nameA], {as:referenceAs, foreignKey: prop})
            }
            break
          default:
        ...
        //console.log(`${nameA}.belongsTo(${nameB}, {as:${associationAs}, foreignKey:${prop}})`)
        this.sequelizeModels[nameA].belongsTo(this.sequelizeModels[nameB], {as:associationAs, foreignKey: prop})
        models[nameB].options.associations.push({model:nameA, as:referenceAs})
      }
    }
  }
      ...
      for (let c = keys.length, n = 0; n < c; n++) {
        ...
        if (Object.keys(modelFile).length > 0) {
          ...
          asyncFunctions.push(function(callback) {
            queryInterface.dropTable(model.name, {cascade:true}).done(result => {
