...
class Database {
  ...
  */
  async setup (moduleName, options = {}) {
    ...
    await this._setupModels(MODULE, ALLOWED_SCHEMAS)
  ...
  * @return {Promise}
  */
  async _setupModels (MODULE, ALLOWED_SCHEMAS) {
    const APP    = this.app
    const SCHEMA = MODULE.config.schema
    const task1  = new Loader(` - CREATE SCHEMA ${SCHEMA}`)
    task1.start()
    await this.sequelize.dropSchema(SCHEMA,   { force: true })
    await this.sequelize.createSchema(SCHEMA, { force: true })
    task1.finish()
    const loaded = []
    APP.log('\n')
    async function _sync (model) {
      if ((!ALLOWED_SCHEMAS.includes(model.options.schema))) { return }
  ...
  */
  async _setupSeeders (MODULE, ALLOWED_SCHEMAS, { seedersPath, seedExt }) {
    ...
    const _seed = async (modelName, filePath) => {
      ...
      if (!ALLOWED_SCHEMAS.includes(model.options.schema)) {
        throw new Error(`No se permite crear el seed '${model.name}' porque pertencece al módulo '${model.options.schema}' cuya instalación ha sido desactivada.`)
      }
      if (model.associations) {
        for (let i in model.associations) {
