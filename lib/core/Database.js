  ...
  */
  async setup (moduleName, options = {}) {
    ...
    if (config.SERVER.env === 'production') {
      ...
      if (util.isDir(options.seedersPath)) await this._setupSeeders(MODULE, ALLOWED_SCHEMAS, options)
    } else {
      options.seedersPath = path.resolve(options.seedersPath, 'development')
      if (util.isDir(options.seedersPath)) await this._setupSeeders(MODULE, ALLOWED_SCHEMAS, options)
    }
  }
