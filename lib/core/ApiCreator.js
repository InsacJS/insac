  ...
  */
  create(app) {
    return new Promise((resolve, reject) => {
      ...
      let indexPath = Util.resolve(this.apidocPath, './index.html')
      ...
      let executeCommand = (command, executePath) => {
        return new Promise((resolve, reject) => {
          exec(command, {cwd: executePath}, (error, stdout, stderr) => {
            if (error !== null) { reject(stderr) } else { resolve() }
          })
        })
      }
      async function init() {
        try {
          ...
          // Creamos el archivo apidoc.json.
          await Util.writeFile(apidocJSONFilename, JSON.stringify(app.config.apidoc))
          // Creamos el archivo que contiene la documentación.
          await Util.writeFile(routesJSFilename, apidocContent)
          // Creamos el archivo header.md para incluirlo en la documentación.
          ...
          // Copiamos el archivo index.html a la carpeta del template
          // Esto es para actualizar la etiqueta <base href="./">
          let indexHTMLContent = require('fs').readFileSync(indexPath, 'utf-8')
          indexHTMLContent = indexHTMLContent.replace(`<base href="./">`,`<base href="${app.config.apidoc.url}/">`)
          await Util.writeFile(`${template}/index.html`, indexHTMLContent)
          // Creamos el apidoc
          ...
          // Eliminamos el archivo index.html generado anteriormente en la carpeta template.
          await executeCommand(`rm -rf index.html`, template)
