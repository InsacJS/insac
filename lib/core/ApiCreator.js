  ...
  */
  create(app) {
    return new Promise((resolve, reject) => {
      ...
      let infoContent = `# Roles de usuario\n`

      // Información sobre los roles de usuarios
      for (let i in app.config.auth.roles) {
        let rol = app.config.auth.roles[i]
        infoContent += `- **${rol.nombre}**. Alias: \`${rol.alias}\`\n`
      }
