...
class Response {
  ...
  */
  success200(data = { message: 'Tarea completada exitosamente' }) {
    let code = (this.all200) ? 200 : 200
    this.status(code).json(Response.formatSuccessJSON(200, data))
  }
  ...
  * @param {Object} data Datos que se incluyen en la respuesta.
  */
  success201(data = { message: 'Registro creado exitosamente' }) {
    let code = (this.all200) ? 200 : 201
    this.status(code).json(Response.formatSuccessJSON(201, data))
  }
  ...
  * @param {String} message Mensaje que detalla el error.
  * @param {String} stack Mensaje adicional acerca del origen del error.
  */
