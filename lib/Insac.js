...
class Insac {
  ...
  */
  addModule (moduleName) {
    ...
    try {
      util.find(app.config.PATH.modules, `${moduleName}.module.js`, ({ filePath, dirPath }) => {
        ...
        if (filesInfo.length > 0) {
          ...
          app.logger.appVerbose('[archivo]', `${filesInfo[0].filePath.replace(app.config.PATH.project, '')} ${app.logger.OK}`)
        }
        ...
        app.logger.appVerbose('[archivo]', `${filePath.replace(app.config.PATH.project, '')} ${app.logger.OK}`)
  ...
  */
  async init () {
    ...
    const SETUP = process.env.SETUP  && (process.env.SETUP === 'true')
    const START = !process.env.START || (process.env.START === 'true')
    ...
    try {
      ...
      if (SETUP) {
        ...
        if (!START) {
          ...
          app.logger.appInfo()
          app.logger.appInfo('La aplicación ha sido instalada con éxito.')
          app.logger.appInfo()
        }
      }
      ...
      app.logger.appFatal('FATAL:', `Ocurrió un error al inicializar la aplicación. ${err.message}\n`)
      throw err
    }
  }
...
async function loadTools (app) {
  const TOOLS_PATH = app.config.PATH.tools
  app.tools = {}
...
async function loadBeforeHooks (app) {
  ...
  if (util.isDir(app.config.PATH.hooks)) {
    await util.findAsync(app.config.PATH.hooks, '.before.hook.js', async ({ filePath, fileName }) => {
  ...
  if (!appHookLoaded) {
    ...
    app.logger.appVerbose('[archivo]', `app.before.hook.js (default) ${app.logger.OK}`)
  }
}
...
async function loadAfterHooks (app) {
  let appHookLoaded = false
  if (util.isDir(app.config.PATH.hooks)) {
    await util.findAsync(app.config.PATH.hooks, '.after.hook.js', async ({ filePath, fileName }) => {
      await require(filePath)(app)
      app.logger.appVerbose('[archivo]', `${filePath.replace(app.config.PATH.project, '')} ${app.logger.OK}`)
      if (fileName === 'app') appHookLoaded = true
    })
  ...
  if (!appHookLoaded) {
    ...
    app.logger.appVerbose('[archivo]', `app.after.hook.js (default) ${app.logger.OK}`)
  }
  app.logger.appVerbose()
}
...
async function listen (app) {
  ...
  if (app.config.SERVER.https === true) {
    app.SERVER = await https.createServer(app.config.SERVER.options, app).listen(app.config.SERVER.port)
  } else {
    app.SERVER = await app.listen(app.config.SERVER.port)
  }
  const URL = `${PROTOCOLO}://localhost:${app.config.SERVER.port}`
  app.logger.appInfo(`[listen]`, `Servicio: ${URL}`)
  if (app.apidoc.isEnabled()) {
    app.getResourceModules().forEach(mod => {
      app.logger.appInfo(`[listen]`, `APIDOC:   ${URL}/apidoc/${mod}`)
    })
    app.logger.appInfo()
  }
  if (!app.apidoc.isEnabled()) {
