  ...
  */
  _setIndexPage (app) {
    ...
    app.get('/', (req, res, next) => {
      ...
      let insacVersion = '1.0.0'
      try { insacVersion = require(insacPackagePath).version } catch (e) {}
      try {
        const APP_PACKAGE_CONTENT = require(appPackagePath)
        appName    = APP_PACKAGE_CONTENT.name
        appVersion = APP_PACKAGE_CONTENT.version
      } catch (e) {}
      const modules = []
      app.MODULES.forEach(moduleName => {
        if (app[moduleName].config.moduleType === 'RESOURCE')  { modules.push(moduleName) }
      })
      const data = { appName, appVersion, insacVersion, modules }
