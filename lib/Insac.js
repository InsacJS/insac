...
async function _listen (app) {
  if (app.config.SERVER.https === true) {
    const options = {
      key  : app.config.SERVER.ssl.key,
      cert : app.config.SERVER.ssl.cert
    }
    https.createServer(options, app).listen(app.config.SERVER.port)
    process.stdout.write(` - Servicio activo: [${app.config.SERVER.env}] https://localhost:${app.config.SERVER.port}\n\n`)
  } else {
    app.listen(app.config.SERVER.port)
    process.stdout.write(` - Servicio activo: [${app.config.SERVER.env}] http://localhost:${app.config.SERVER.port}\n\n`)
  }
}
