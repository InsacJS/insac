...
class Insac {
  ...
  addModule (moduleName, options = {}) {
    util.find('src/modules', '.module.js', ({ filePath, fileName, dirPath }) => {
      ...
      if (MODULE_NAME === moduleName.toUpperCase()) {
        ...
        CONFIG.setupEnabled = options.setupEnabled
  ...
  */
  async init () {
    const BEFORE_PATH = path.resolve(process.cwd(), 'src/hooks/before.js')
    const AFTER_PATH  = path.resolve(process.cwd(), 'src/hooks/after.js')
    try {
  ...
  */
  async _setup (app) {
    ...
    for (let i in app.MODULES) {
      ...
      if (((modules.length === 0) || modules.includes(MODULE_NAME)) && app[MODULE_NAME].config.setupEnabled) {
  ...
  */
  async _start (app) {
    ...
    if (process.env.APIDOC && process.env.APIDOC === 'true') {
      const task1 = new Loader(` Create APIDOC`)
      task1.start()
