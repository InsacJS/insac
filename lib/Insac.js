...
class Insac {
  ...
  */
  constructor(projectPath = Config.DEFAULT_PROJECT_PATH) {
    ...
    if (!global.CLI) console.log(`\x1b[32m\n |==========================================|`)
    if (!global.CLI) console.log(` |===  \x1b[33m I N S A C    F R A M E W O R K \x1b[32m  ===|`)
    if (!global.CLI) console.log(` |==========================================|\n\x1b[0m`)
  ...
  */
  _createConfig(projectPath) {
    ...
    let fileName = `.insac`
    let configuraciones = Util.getContentFiles(projectPath, {fileName:fileName, recursive:false})
    // Configuración por defecto.
    let config = new Config({env:env, projectPath:projectPath})
    // Si existe el archivo de configuración, se actualiza la configuración por defecto.
    if (Object.keys(configuraciones).length == 1) {
      config.update(configuraciones[fileName])
    }
    return config
  }
  ...
  */
  addModel(model) {
    ...
    // Se inicializa el modelo, actualiza las referencias que tiene con otros modelos.
    model.init(this.models)
    ...
    if (!global.CLI) console.log(`\x1b[2m - ${model.name}\x1b[0m`)
  }

  /**
  * Adiciona los modelos dinamicamente a partir de los archivos que se encuentran dentro
  * del directorio models. Si un modelo requiere de otro, previamente se adicionará el modelo requerido.
  */
  addModels() {
    if (!global.CLI) console.log(` Adicionando modelos ...\n`)
    let contentFiles = Util.getContentFiles(this.config.path.models)
    for (let modelName in contentFiles) {
      this._loadModel(modelName, contentFiles)
    }
    if (!global.CLI) console.log(`\x1b[32m\n Modelos adicionados exitosamente\x1b[0m \u2713\n`)
  }

  /**
  * Adiciona un modelo a la colección de modelos. Si el modelo tuviera alguna
  * referencia hacia otro modelo que no haya sido adicionado, previamente se
  * adicionará el modelo requerido.
  */
  _loadModel(modelName, contentFiles) {
    let contentFile = contentFiles[modelName]
    if (typeof contentFile == 'undefined') {
      throw new Error(`Ǹo existe el modelo requerido`)
    }
    try {
      let model = contentFile(this)
      if (!this.models[modelName]) {
        this.addModel(model)
      }
    } catch(err) {
      if (err.message.startsWith('MODEL_NOT_FOUND')) {
        let modelRequiredName = (err.message.split(' '))[1]
        this._loadModel(modelRequiredName, contentFiles)
        this._loadModel(modelName, contentFiles)
      } else {
        console.log(err)
  ...
  */
  addMiddlewares() {
    ...
    for (let name in files) {
      let middleware = files[name](this, this.models, this.db())
      if (Array.isArray(middleware)) {
        for (let j in middleware) {
          let mw = middleware[j]
  ...
  */
  addSeeders() {
    const fs = require('fs')
    let seedersPath = `${this.config.path.seeders}/${this.config.env}`
    try {
  ...
  */
  addRoute(route) {
    ...
    if (!global.CLI) console.log(`\x1b[2m ${route.ID()}\x1b[0m`)
  }
  ...
  */
  addRoutes() {
    if (!global.CLI) console.log(` Adicionando rutas ...\n`)
  ...
  */
  addResources() {
    if (!global.CLI) console.log(` Adicionando recursos ...`)
