...
class Reference extends Field {
  ...
  */
  constructor(name, field, reference) {
    ...
    let as = reference.as || ((type == ONE_TO_ONE) ? model : `${model}s`) // Este valor siempre es en singular
  ...
  */
  ...
  // Devuelve el objeto output hijo, de un objeto output padre, de acuerdo al tipo de referencia.
  referenceOutput(output) {
    let referenceOutput
    if (this.reference.type == ONE_TO_ONE) {
      if (Array.isArray(output[this.reference.model])) {
        throw new Error(`El campo ${this.reference.model} debe ser un objeto`)
      }
      referenceOutput = output[this.reference.model]
    }
    if (this.reference.type == ONE_TO_MANY) {
      //console.log("MANY = ", output)
      // if (!Array.isArray(output[this.reference.model])) {
      //   throw new Error(`El campo ${this.reference.model} debe ser un Array`)
      // }
      referenceOutput = output[this.reference.model]
    }
