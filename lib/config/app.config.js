...
/** @ignore */ const SOURCES_PATH = path.resolve(PROJECT_PATH, 'src')
...
function getConfigFile (fileName) {
  let filesInfo = util.find(SOURCES_PATH, `${fileName}.config.js`)
...
function setNodeEnv (fileName, config) {
  switch (fileName) {
    case 'logger':
      ...
      if (process.env.LOG_LEVEL) { config.console.transport.level = process.env.LOG_LEVEL         } else { process.env.LOG_LEVEL = `${config.console.transport.level}` }
      break
    case 'server':
