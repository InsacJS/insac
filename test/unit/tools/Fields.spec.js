...
describe('\n - Clase: Fields\n', () => {
  ...
  describe(` Método (static): THIS`, () => {
    it('Verificando valor', () => {
      let otherField = new Field({allowNull:false,description:'descripción'})
      let func = Fields.THIS({allowNull:true})
      expect(typeof func).to.equal('function')
      let newField = func(otherField)
      expect(newField instanceof Field).to.equal(true)
      expect(newField.allowNull).to.equal(true)
  ...
  describe(` Método (static): COPY`, () => {
    it('Verificando valor', () => {
      ...
      expect(newField instanceof Field).to.equal(true)
      expect(newField.allowNull).to.equal(false)
      expect(newField.description).to.equal('descripción')
    })
  })
  ...
  describe(` Método (static): STRING`, () => {
    ...
    it('Verificando la instancia STRING con 1 argumento 2da forma', () => {
      let field = Fields.STRING({allowNull:false})
      expect(field instanceof Field).to.equal(true)
      expect(field.type instanceof STRING).to.equal(true)
      expect(field.allowNull).to.equal(false)
    })
  ...
  describe(` Método (static): INTEGER`, () => {
    ...
    it('Verificando la instancia INTEGER con argumentos', () => {
      let field = Fields.INTEGER({allowNull:false, description:'custom'})
      expect(field instanceof Field).to.equal(true)
      expect(field.type instanceof INTEGER).to.equal(true)
  ...
  describe(` Método (static): ONE_TO_ONE`, () => {
    it('Verificando la instancia ONE_TO_ONE con argumentos 1ra forma', () => {
      let usuario = new Model('usuario')
      let field = Fields.ONE_TO_ONE(usuario)
      expect(field instanceof Field).to.equal(true)
      expect(field instanceof Reference).to.equal(true)
      expect(field.validator instanceof Validator).to.equal(true)
      ...
      expect(field.reference.type).to.equal(Reference.ONE_TO_ONE)
    })
    it('Verificando la instancia ONE_TO_ONE con argumentos 2da forma', () => {
      let usuario = new Model('usuario')
      let field = Fields.ONE_TO_ONE(usuario, {allowNull:false, description:'custom'})
      expect(field instanceof Field).to.equal(true)
      expect(field instanceof Reference).to.equal(true)
      expect(field.validator instanceof Validator).to.equal(true)
      expect(typeof field.reference).to.equal('object')
  ...
  describe(` Método (static): ONE_TO_MANY`, () => {
    it('Verificando la instancia ONE_TO_MANY con argumentos', () => {
      let usuario = new Model('usuario')
      let field = Fields.ONE_TO_MANY(usuario)
      expect(field instanceof Field).to.equal(true)
      expect(field instanceof Reference).to.equal(true)
      expect(field.validator instanceof Validator).to.equal(true)
      expect(typeof field.reference).to.equal('object')
      ...
      expect(field.reference.as).to.equal('usuario')
      expect(field.reference.key).to.equal('id')
      expect(field.reference.type).to.equal(Reference.ONE_TO_MANY)
    })
      it('Verificando la instancia ONE_TO_MANY con argumentos', () => {
        let usuario = new Model('usuario')
        let field = Fields.ONE_TO_MANY(usuario, {allowNull:false, as:'usuario_custom', description:'custom'})
        expect(field instanceof Field).to.equal(true)
        expect(field instanceof Reference).to.equal(true)
        expect(field.validator instanceof Validator).to.equal(true)
        expect(typeof field.reference).to.equal('object')
        expect(field.reference.model).to.equal('usuario')
        expect(field.reference.as).to.equal('usuario_custom')
        expect(field.reference.key).to.equal('id')
        expect(field.reference.type).to.equal(Reference.ONE_TO_MANY)
        expect(field.allowNull).to.equal(false)
        expect(field.description).to.equal('custom')
