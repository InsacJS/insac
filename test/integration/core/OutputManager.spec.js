...
describe('\n - Clase: OutputManager\n', () => {
  ...
  it('Verificando la propiedad queryOptions para el modelo administrativo 1', () => {
    ...
    options = {
      ...
      include: [
        {attributes:['ci','id_usuario','id'], model:db.persona, as:'persona'}
      ]
    }
    expect(_.isEqual(optionsOptimized, options)).to.equal(true)
    ...
    options = {
      attributes: ['cargo','id_persona','id'],
      include: [
        {attributes:['ci','id_usuario','id'], model:db.persona, as:'persona', include:[
          {attributes:['username','id'], model:db.usuario, as:'usuario', include: [
            {attributes:['estado','id_usuario','id_rol','id'], model:db.rol_usuario, as:'roles_usuarios', include:[
