...
describe('\n - Clase: OutputManager\n', () => {
  ...
  it('Verificando la propiedad queryOptions para el modelo administrativo 1', () => {
    ...
    optionsOptimized = outputManager.getOptions(req, app.routes[route.ID()], app.models)
    options = {
      attributes: ['cargo','id','id_persona'],
      include: [
        {attributes:['ci','id','id_usuario'], model:db.persona, as:'persona'}
      ]
    }
    expect(_.isEqual(optionsOptimized, options)).to.equal(true)
    ...
    options = {
      attributes: ['cargo','id','id_persona'],
      include: [
        {attributes:['ci','id','id_usuario'], model:db.persona, as:'persona', include:[
          {attributes:['username','id'], model:db.usuario, as:'usuario', include: [
            {attributes:['estado','id','id_usuario','id_rol'], model:db.rol_usuario, as:'roles_usuarios', include:[
    ...
    options = {
      attributes: ['id','cargo','id_persona'],
      include: [
        {attributes:['id','nombre','ci','id_usuario'], model:db.persona, as:'persona', include:[
